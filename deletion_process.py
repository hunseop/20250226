...
def add_duplicate_status():
    """
    중복정책 분류 결과(작업구분)를 정책 파일에 추가하는 함수
    """
    print("정책 파일을 선택하세요:")
    policy_file = select_xlsx_files()
    if not policy_file:
        return False
    
    print("중복정책 분류 결과 파일(_정리.xlsx)을 선택하세요:")
    duplicate_file = select_xlsx_files()
    ...
    try:
        ...
        # 결과 저장
        output_file = update_version(policy_file)
        ...
        logging.exception(f"중복여부 정보 추가 중 오류 발생: {e}")
        return False

def add_usage_status():
    """
    미사용 정책 정보를 정책 파일에 추가하는 함수
    """
    print("정책 파일을 선택하세요:")
    policy_file = select_xlsx_files()
    if not policy_file:
    ...
    try:
        ...
        logging.exception(f"미사용여부 정보 추가 중 오류 발생: {e}")
        return False
...
def select_task():
    ...
    print("10. 미사용 정책 정보를 정책 파일에 추가하기")
    print("0. 종료")

    while True:
        try:
            choice = int(input("작업 번호 (1-10, 종료: 0)"))
            if 1 <= choice <= 10:
                return choice
            elif choice == 0:
                exit('exit')
